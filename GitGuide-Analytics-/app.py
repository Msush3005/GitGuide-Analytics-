...
Now includes Live GitHub Repository URL ingestion:
  - Enter any public GitHub repo URL (e.g. https://github.com/facebook/react)
  - Click 'Fetch Live GitHub Data' to generate a CSV dataset automatically
  - Dashboard auto-refreshes with real contributor and PR metrics
...
# Add scripts/ to Python path so github_repo_ingestion can be imported
...
def main():
    ...
    # 1. Live GitHub Repository URL Ingestion
    st.sidebar.markdown("**🔗 Live GitHub Repository**")
    github_url = st.sidebar.text_input(
        "Enter GitHub Repo URL or owner/repo:",
    ...
    if fetch_clicked and github_url.strip():
        with st.spinner(f"Fetching data from GitHub: {github_url.strip()}..."):
            try:
                ...
                st.sidebar.success(
                    f"✓ Fetched {report['total_contributors']} contributors, "
                    f"{report['total_commits_fetched']} commits from {report['repository']}"
                )
                df = df_fetched
            except Exception as e:
                st.sidebar.error(f"GitHub fetch failed: {e}")
                df = load_default_dataset()
    elif os.path.exists(fetched_csv) and not fetch_clicked:
        try:
            df = pd.read_csv(fetched_csv)
            repo_name = df['repository_name'].iloc[0] if 'repository_name' in df.columns and len(df) > 0 else 'GitHub repo'
            st.sidebar.info(f"📂 Loaded previously fetched GitHub data: **{repo_name}** ({len(df)} contributors)")
        except Exception:
            df = load_default_dataset()
    else:
        # 2. Manual CSV Upload Fallback
        st.sidebar.markdown("**📁 Or Upload CSV Manually**")
